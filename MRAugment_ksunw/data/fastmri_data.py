...
class SliceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        root: Union[tuple[str], tuple[Path], tuple[os.PathLike], List[tuple[str]], List[tuple[Path]]],
        challenge: str,
        transform: Optional[Callable] = None,
        use_dataset_cache: bool = False,
        sample_rate: Optional[float] = None,
        volume_sample_rate: Optional[float] = None,
        dataset_cache_file: Union[str, Path, os.PathLike] = "../../../input/dataset_cache.pkl",
        num_cols: Optional[Tuple[int]] = None,
        scanner_models: Optional[List[str]] = None
    ):
        """
        Args:
            root: tuple of Path to the dataset - (kspace_datapath, image_datapath)
        ...
        if not_cached or not use_dataset_cache:
            ...
            if isinstance(root, list):
                files = []
                for r in root:
                    files += list(zip([list(Path(f).iterdir()) for f in r])) #list(Path(r).iterdir())
            else:        
                files = list(zip([list(Path(f).iterdir()) for f in root])) #list(Path(root).iterdir())
            
            for fname in sorted(files):
                metadata, num_slices = self._retrieve_metadata(fname[0])
    ...
    def __getitem__(self, i: int):
        (fname_k, fname_i), dataslice, metadata = self.examples[i] #fname1: kspace, fname2: image

        with (h5py.File(fname_k, "r"), h5py.File(fname_i, "r")) as (hf_k, hf_i):
            kspace = hf_k["kspace"][dataslice]

            mask = np.asarray(hf_k["mask"]) if "mask" in hf_k else None

            target = hf_i["image_label"][dataslice] if "image_label" in hf else None
            #target = hf[self.recons_key][dataslice] if self.recons_key in hf else None

            attrs = dict(hf_i.attrs)
            #attrs.update(metadata)

        if self.transform is None:
            sample = (kspace, mask, target, attrs, fname_k.name, dataslice)
        else:
            sample = self.transform(kspace, mask, target, attrs, fname_k.name, dataslice)
