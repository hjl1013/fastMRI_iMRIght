...
def augment_image_from_kspace(kspace, target_size, max_train_size=None):
    im = ifft2c(kspace)
...
def random_apply(transform_name):
    weight_dict = {'fliph': 0.1, 'flipv': 0.1, 'rot90': 0.1, }
    rng = np.random.RandomState()
    if rng.uniform() < weight_dict[transform_name] * augmentation_strength:
        return True
    else:
        return False
...
class AugmentationPipeline:
    ...
    # 새로 만든 함수
    def augment_from_image(self, image, target_size, max_train_size=None):
        image = self.augment_image(image, max_output_size=max_train_size)
        target = self.im_to_target(image, target_size)
        return image, target
...
class DataAugmentor:
    ...
    def __call__(self, image, target_size):
        """
        ...
        if self.aug_on and p > 0.0:
            kspace, target = self.augmentation_pipeline.augment_image(image,
