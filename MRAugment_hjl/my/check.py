...
from MRAugment_hjl.data.transforms import complex_center_crop
...
#
# with h5py.File("/root/fastMRI/result/VarNet_pretrained/reconstructions/brain102.h5", "r") as hf:
#     recon_image = hf['reconstruction'][0]
# with h5py.File("/root/input/image/multicoil_val/brain102.h5", "r") as hf:
#     target_image = hf['image_label'][0]
#
# plt.figure()
# plt.subplot(121)
...
def data_reconstruct():
    for dir_name in Path("/root/input/kspace").iterdir():
        for fname in dir_name.iterdir():
            with h5py.File(str(fname), "r") as hf_k, h5py.File(str(Path("/root/input/image") / dir_name.name / fname.name), "r") as hf_i, h5py.File(str(Path("/root/input_recon/kspace") / dir_name.name / fname.name), "w") as whf:
                for key in hf_k:
