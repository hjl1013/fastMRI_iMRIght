...
from ADL_utils.dataloader import DataLoader_cls
from model.metric import MetricEval
from ADL_utils import util
from model import loss
from utils.common.loss_function import SSIMLoss

from utils.data.load_data import create_data_loaders
from Code.train import parse

import subprocess

#dir_path = os.path.dirname(os.path.abspath(__file__))
dir_path = '/root/result'
...
parser.add_argument('--train-dirs', nargs="+", default='trainDir', required=False,
						help="Imoport your datasets. The datasets can be separated by comma")
parser.add_argument('--test-dirs', nargs="+", default='TestDir', required=False, help="...")
...
def main(args, config_data, params, PHASE):
	...
	'''
	ds_train_loader = create_data_loaders(data_path=args.data_path_train, args=args, use_augment=True)
	ds_valid_loader = create_data_loaders(data_path=args.data_path_val, args=args, use_augment=False)
	ds_test_loader = create_data_loaders(data_path=args.data_path_test, args=args, use_augment=False)
	...
	if PHASE == 1:
		...
		denoiser_module= Denoiser_Trainer(params, device,
								args.CHANNELS_NUM, args.CHANNELS_NUM, # TODO different in_chans & out_chans
		...
		discriminator_module= Discriminator_Trainer(params, device,
								args.CHANNELS_NUM, args.CHANNELS_NUM, # TODO both out_chans
								writer_numerical, writer_imgs, distributed=False)
		...
		adl_module= ADL_Trainer(params, device,
								args.CHANNELS_NUM, args.CHANNELS_NUM, # TODO
								writer_numerical, writer_imgs, distributed=False)
...
if __name__== '__main__':
	...
	loss_denoiser_fns = {
		'L1': loss.Loss_L1, # TODO change to SSIM
	...
	eval_params_denoiser = {
		#'psnr': MetricEval.psnr, # TODO change to SSIM
		'ssim_loss': SSIMLoss()
	}
