...
class Trainer(abc.ABC):
            ...
            for i, batch_i in enumerate(train_data):
                ...
                if  (step_i>0) and (step_i%validation_per_step == 0):
                    if  len(validation_data)>0:
                        ...
                        #self.scheduler.step(summary_val['ssim_loss']) TODO use ReduceLROnPlateau
                ...
                if (self.rank == 0) and (step_i>0) and (step_i%ckpts_per_step==0):
                    if self.mode == 'ADL': # TODO check how to save ADL
    ...
    def _get_val(self, model, validation_data, step_i):
        ...
        with torch.no_grad():
            for i, batch_i in enumerate(validation_data):
                ...
                min = batch_i['min'].cuda(self.device)
                max = batch_i['max'].cuda(self.device)
                ...
                y_pred = min + y_pred*(max-min)

                # apply evaluation metrics
                for name, fn in self.metrics.items():
                    val = fn.cuda(self.device)(GT,y_pred, batch_i['maximum'].cuda(self.device))
                    summary[name]+=val
                    if self.rank == 0:
    ...
    #  auxiliary funcs
    def _save_model(self, model, step_i, optimizer): #, scheduler): TODO save state_dict & scheduler
